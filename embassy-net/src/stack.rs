...
impl<D: Device + 'static> Stack<D> {
    ...
    pub fn ethernet_address(&self) -> [u8; 6] {
        unsafe { self.with(|s, i| i.device.device.ethernet_address()) }
    }

    pub fn is_link_up(&self) -> bool {
        unsafe { self.with(|s, i| i.link_up) }
    }

    pub fn is_config_up(&self) -> bool {
        unsafe { self.with(|s, i| i.config.is_some()) }
    }

    pub fn config(&self) -> Option<Config> {
        unsafe { self.with(|s, i| i.config.clone()) }
    }
